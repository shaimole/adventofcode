...
fn init_edges(
    mut caves: HashMap<String, Vec<String>>,
...
fn traverse_2(
    current_cave: &String,
    target_cave: &String,
    visited_once: &Vec<String>,
    caves: HashMap<String, Vec<String>>,
    visited_twice: &Vec<String>,
) -> u32 {
    if current_cave == target_cave {
        return 1;
    }
    ...
    match caves.get(current_cave) {
        Some(connections) => {
            for connection in connections.to_owned() {
                ...
                if (visited_once.contains(&connection) && new_visited_twice.len() > 0)
                    || connection == "start"
                {
                    continue;
                }
                total_paths += traverse_2(
                    &connection,
                    target_cave,
                    &new_visited_once,
                    caves.to_owned(),
