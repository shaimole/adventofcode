...
mod tests {
    ...
    fn it_should_find_neighbours() {
        assert_eq!(
            get_neighbours((0, 0), 1, 1).sort_unstable(),
            vec![(0, 1), (1, 0)].sort_unstable()
        );
        assert_eq!(
            get_neighbours((2, 2), 3, 3).sort_unstable(),
            vec![(2, 1), (2, 3), (1, 2), (3, 2)].sort_unstable()
        );
    }
    #[test]
    fn it_should_get_correct_elevation() {
        assert_eq!(
            get_elevation(&"a".to_string()),
            get_elevation(&"S".to_string())
        );
        assert_eq!(
            get_elevation(&"a".to_string()),
            get_elevation(&"b".to_string()) - 1
        );
    }
    #[test]
