...
fn init_monkeys_sample() -> Vec<Monkey<dyn Fn(usize) -> (usize, u32)>> {
    let felix_ops = Box::new(|item: usize| {
    ...
    let lars_ops = Box::new(|item: usize| {
    ...
    let lars = Monkey {
        items: vec![vec![54; 4], vec![65; 4], vec![75; 4], vec![74; 4]],
        operation: lars_ops,
    };

    let jens_ops = Box::new(|item: usize| {
    ...
    let jens = Monkey {
        items: vec![vec![79; 4], vec![60; 4], vec![97; 4]],
        operation: jens_ops,
    };

    let raik_ops = Box::new(|item: usize| {
...
fn init_monkeys() -> Vec<Monkey<dyn Fn(usize) -> (usize, u32)>> {
    ...
    let lars = Monkey {
        items: vec![vec![75; 8], vec![83; 8], vec![74; 8]],
        operation: lars_ops,
    };
    let jens = Monkey {
        items: vec![
    ...
    let tom_ops = Box::new(|item: usize| {
        let modulo = 11;
        let success = 3;
        let fail = 1;
    ...
    let leo_ops = Box::new(|item: usize| {
        let modulo = 19;
        let success = 6;
        let fail = 2;
    ...
    let alfred_ops = Box::new(|item: usize| {
        let modulo = 5;
        let success = 2;
        let fail = 7;
    ...
    let sepp_ops = Box::new(|item: usize| {
        let modulo = 13;
        let success = 4;
        let fail = 0;
